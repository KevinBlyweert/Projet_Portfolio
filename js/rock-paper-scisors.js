...
function checkWinner() {
    computerChoice = utilities.randomNumber(0, 2);
...
function createList(player, parentClass) {
    const list = document.createElement('ul');
    document.querySelector(parentClass).append(list);
    // document.querySelector(parentClass).classList.add('RPS');
...
export function RPS(parentClass) {
    playerScore = 0; computerScore = 0;
    launchWord = document.createElement('div')
    launchWord.textContent = items[0]
    launchWord.classList.add('launchword')
    launchWord.classList.add('hidden')
    document.querySelector(parentClass).append(launchWord);
    scores = document.createElement('div')
    scores.textContent = ""
    scores.classList.add('scores')
    scores.classList.add('hidden')
    document.querySelector(parentClass).append(scores);
    players.forEach(player => {
        readyToPlay = true
        createList(player,parentClass);
    })
}